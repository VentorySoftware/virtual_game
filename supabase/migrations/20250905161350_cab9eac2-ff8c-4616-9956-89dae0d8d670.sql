...
-- Insert preorder products
UPDATE public.products SET type = 'preorder', preorder_date = '2024-10-20T00:00:00Z' WHERE slug = 'spider-man-2-ps5';
...
INSERT INTO public.product_reviews (product_id, user_id, rating, title, content, is_approved, is_featured, created_at) VALUES
((SELECT id FROM public.products WHERE slug = 'cyberpunk-2077-phantom-liberty' LIMIT 1), '00000000-0000-0000-0000-000000000001', 5, 'Increíble expansión', 'Phantom Liberty es exactamente lo que Cyberpunk necesitaba. Una historia emocionante con Keanu Reeves.', true, true, now() - INTERVAL '5 days'),
((SELECT id FROM public.products WHERE slug = 'baldurs-gate-3' LIMIT 1), '00000000-0000-0000-0000-000000000002', 5, 'El mejor RPG en años', 'BG3 establece un nuevo estándar para los RPGs. Cada decisión importa y la libertad es total.', true, true, now() - INTERVAL '3 days'),
((SELECT id FROM public.products WHERE slug = 'starfield' LIMIT 1), '00000000-0000-0000-0000-000000000003', 4, 'Ambicioso pero con defectos', 'Un juego masivo con exploración increíble, aunque algunos aspectos podrían mejorarse.', true, false, now() - INTERVAL '2 days'),
((SELECT id FROM public.products WHERE slug = 'elden-ring' LIMIT 1), '00000000-0000-0000-0000-000000000004', 5, 'Obra maestra absoluta', 'FromSoftware ha creado su mejor juego. Desafiante, hermoso e inolvidable.', true, true, now() - INTERVAL '1 day'),
((SELECT id FROM public.products WHERE slug = 'witcher-3-goty' LIMIT 1), '00000000-0000-0000-0000-000000000005', 5, 'Atemporal', 'Años después sigue siendo el mejor RPG. Con todas las expansiones es una experiencia completa.', true, true, now() - INTERVAL '6 hours');
